...
BASE_DATA_PATH = os.path.join(Path.cwd().parent, 'Data', 'Output')
...
def generate_expectancy_matrix(year: str):
    ...
    outs_scores = run_expectancy.stitch_data(str_data, int_data)
    matrix = run_expectancy.build_run_ex_matrix(outs_scores)
...
def generate_event_values(year: str):
    t = time.time()
    str_data, int_data = read_raw_data(year)
    situation_events = event_values.stitch_data(str_data, int_data)
    matrix = load_run_matrix(year)
    values = event_values.get_event_values(situation_events, matrix)
...
def get_player_batting_runs(year: str):
    ...
    batting_runs = player_stats.get_batter_stats(str_data, int_data, weights, park_factors)
